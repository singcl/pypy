...
class M3U8Download:
        ...
        try:
            ...
            with requests.get(source_url) as r:  # 访问资源控制文件,获得资源信息
                ...
                for sub_src in src:  # 将资源地址储存到任务字典
                    if sub_src:
                        self._target_url[sub_src] = self._url_seed + \
                            "/" + sub_src
        except Exception as e:
            print("资源无法成功解析", e)
            os.system("pause")
    ...
    def _multiprocessing(self, processing_num=4):  # 多进程,多线程
        ...
        for path, url in self._target_url.items():  # 分配进程任务
            ...
            if i % 10 == 0 or i == len(self._target_url):  # 每个进程分配十个任务
                if self._mode == "2":
                    pool.apply_async(self._sub_multithreading,
                                     kwds=target_list)  # 使用多线程驱动方法
                else:
                    pool.apply_async(self._sub_coroutine,
                                     kwds=target_list)  # 使用协程驱动方法
                target_list = {}
    ...
    def _download(self, path, url):  # 同步下载方法
        with requests.get(url, headers=self._headers) as r:
            if r.status_code == 200:
                ...
                print(len(self._ml), ": ", path, "->OK",
                      "\tcomplete:", percent, "%")  # 显示下载进度
            else:
    ...
    def _combine(self):  # 组合资源方法
        try:
            ...
            exec_str = r'copy /b  "' + self._path + r'*.ts" "' + \
                self._path + 'video' + identification + '.mp4"'
